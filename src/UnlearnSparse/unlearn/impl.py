...
def save_unlearn_checkpoint(model, evaluation_result, args):
    ...
    utils.save_checkpoint(state, False, args.save_dir, args.unlearn, filename=f'checkpoint_{args.seed}.pth.tar')
    utils.save_checkpoint(evaluation_result, False, args.save_dir,
                          args.unlearn, filename=f"eval_result_{args.seed}.pth.tar")
