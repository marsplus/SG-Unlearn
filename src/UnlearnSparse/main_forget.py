...
def main():
    ...
    # # prepare dataset
    # _, train_loader_full, val_loader, test_loader, marked_loader = utils.setup_model_dataset(
    #     args)
    # # model.cuda()
    # # breakpoint()
    ...
    # output_data = {}
    # for key in unlearn_data_loaders.keys():
    #     output_data[key] = {
    #         'dataset': unlearn_data_loaders[key].dataset,
    #         'batch_size': unlearn_data_loaders[key].batch_size
    #     }
    ...
    retain_loader = torch.utils.data.DataLoader(
        retain_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, generator=RNG)
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2, generator=RNG)
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, generator=RNG)
    forget_loader = torch.utils.data.DataLoader(
        forget_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, generator=RNG)
