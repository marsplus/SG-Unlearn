...
def main(args):
    ...
    if args.dataset != "20ng":
        ...
        adv_batch_size = int(len(held_out_wo_class) / args.mem_save)
        test_set, val_set = random_split(held_out_wo_class, [0.5, 0.5], generator=RNG)
    ...
    torch.save(
        ...
        os.path.join(
            args.output_dir,
            f"SGdata_seed_{args.seed}_class_{args.forget_class}_{args.dataset}.pth",
        ),
    )
