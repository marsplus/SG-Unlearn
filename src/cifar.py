...
def main(args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_set = torchvision.datasets.SVHN("../data", split='train', transform=transform_train, download=True)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=num_workers)
        held_out = torchvision.datasets.SVHN("../data", split='test', transform=transform_test, download=True)
        args.num_class = 10
    ...
    test_loader = DataLoader(test_set, batch_size=adv_batch_size, shuffle=False, num_workers=num_workers)
    val_loader = DataLoader(val_set,   batch_size=adv_batch_size, shuffle=True, num_workers=num_workers)
    ## construct retain and forget sets
    forget_set, retain_set = random_split(train_set, [0.1, 0.9], generator=RNG)
    forget_loader = torch.utils.data.DataLoader(
        forget_set, batch_size=adv_batch_size, shuffle=True, num_workers=num_workers, generator=RNG)
    ...
    torch.save(SG_data, os.path.join(args.output_dir, f'SGdata_seed_{args.seed}_{args.dataset}.pth'))
    ...
    if args.arch == 'resnet18':
        if args.dataset == 'cifar10':
            ...
            local_path = args.model_path if args.model_path else os.path.join(ROOT_DIR, "../models/resnet18-200-regular.pth")
        elif args.dataset == 'svhn':
            local_path = args.model_path if args.model_path else os.path.join(ROOT_DIR, "../models/svhn_ckpt.pth")
        else:
